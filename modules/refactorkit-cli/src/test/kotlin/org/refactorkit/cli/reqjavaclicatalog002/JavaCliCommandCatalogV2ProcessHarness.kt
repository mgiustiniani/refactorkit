...
/**
 * Isolates the JDK 21 SecurityManager tripwire in a child JVM whose required startup option is explicit.
 * The production CLI remains unchanged and receives no fault-injection seam.
 */
internal object JavaCliCommandCatalogV2ProcessHarness {
    private const val HARNESS_PROTOCOL = "refactorkit.test.catalog-v2-guard/v1"
    ...
    fun main(arguments: Array<String>) {
        ...
        lateinit var manager: CatalogBoundarySecurityManager
        ...
        try {
            ...
            manager = CatalogBoundarySecurityManager(repositoryRoot, installedRoot)
            manager.checkRead(repositoryRoot.resolve("build/catalog-guard-warmup").toString())
            probeBoundary(manager, installedRoot.resolve("bin/refactorkit"))
    ...
    private fun probeBoundary(manager: CatalogBoundarySecurityManager, installedExecutable: Path) {
        assertBlocked(manager, "exec", installedExecutable)
        assertBlocked(manager, "read", installedExecutable)
        assertBlocked(manager, "write", installedExecutable)
        assertBlocked(manager, "delete", installedExecutable)
    }

    private fun assertBlocked(manager: CatalogBoundarySecurityManager, kind: String, path: Path) {
        try {
            when (kind) {
                "exec" -> manager.checkExec(path.toString())
                "read" -> manager.checkRead(path.toString())
                "write" -> manager.checkWrite(path.toString())
                "delete" -> manager.checkDelete(path.toString())
                else -> error("unsupported tripwire kind: $kind")
            }
            error("$kind tripwire action was not blocked")
        } catch (_: SecurityException) {
            // Expected: the probe verifies the same fail-closed checks used around each CLI invocation.
        }
    }
...
@Suppress("DEPRECATION")
internal class CatalogBoundarySecurityManager(
    ...
    private fun isBuildInfrastructure(path: Path): Boolean {
        ...
        for (segment in relative) {
            ...
            if (value == "build" || value == ".gradle") return true
        }
        return false
    }

    private fun normalized(raw: String?): Path? {
