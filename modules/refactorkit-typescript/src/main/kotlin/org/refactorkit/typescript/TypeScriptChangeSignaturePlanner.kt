...
/**
 * Change signature of a TypeScript/JavaScript method/function using the TypeScript language server.
 */
class TypeScriptChangeSignaturePlanner(
    private val client: TypeScriptSemanticClient,
) {
    fun preview(snapshot: ProjectSnapshot, filePath: Path, symbolName: String, newSignature: String): PatchPlan {
        ...
        val escapedSignature = newSignature.replace("\"", "\\\"")
        val paramsJson = """{"method":"getRefactorEdits","params":{"textDocument":{"uri":"$uri"},"refactor":"changeSignature","action":"Change signature","arguments":{"symbolName":"$escapedSymbol","newSignature":"$escapedSignature","actionName":"Change signature"},"interactive":false}}"""
