...
/** Stable T5 refusal until declaration-family, overload, override and call-site authority is proven. */
class TypeScriptChangeSignaturePlanner(private val client: TypeScriptSemanticClient) {
    fun preview(snapshot: ProjectSnapshot, filePath: Path, symbolName: String, newSignature: String): PatchPlan {
        ...
        return refused(snapshot, "typescript.changeSignatureUnsupported",
            "TypeScript change signature has no qualified declaration-family, overload, override and call-site authority")
    }

    private fun refused(snapshot: ProjectSnapshot, code: String, reason: String) = PatchPlan(
        operation = "changeSignature", status = PatchStatus.REFUSED, snapshotHash = snapshot.hash,
