...
/**
 * Story BDD glue for features/java-inline-variable-refusal.feature
...
class KotlinJvmInlineVariableRefusalSteps(private val context: KotlinJvmRefusalScenarioContext) {
    ...
    private val refusalCode = "java.inlineVariable.unsupported"

    // ---------------------------------------------------------- Scenario 1 Given

    @Given("^the Java adapter receives an inlineVariable refactoring request on a Java workspace$")
    fun javaAdapterReceivesInlineVariableRequest() {
        setupFixture()
    ...
    private fun setupFixture() {
        val root = temporaryDirectory("rk-jvm-inline-var-refusal")
    ...
    fun cleanup(scenario: Scenario) {
        ...
        val report = reportDir.resolve("java-inline-variable-refusal-codes.txt")
