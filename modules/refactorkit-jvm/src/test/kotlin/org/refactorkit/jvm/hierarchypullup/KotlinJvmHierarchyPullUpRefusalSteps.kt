...
/**
 * Story BDD glue for features/java-hierarchy-pull-up-refusal.feature
 * (REQ-JAVA-HIERARCHY-PULL-UP-REFUSAL-001, row N-PULL-UP of the approved J1 catalogue).
 *
 * It drives the real production JavaLanguageAdapter.applyRefactoring with a pullUpMember
 * request on a minimal Java workspace and asserts that the DECLARED typed refusal code
 * java.hierarchy.pullUp.unsupported EQUALS the ACTUAL refusalCode the adapter returned.
 * Before the production fix this suite is RED: the adapter has no pullUpMember branch and
 * falls through to the generic notImplemented fallback, so refusalCode stays null and the
 * declared-code assertion fails. After the minimal production fix (an explicit pullUpMember
 * branch returning REFUSED with the stable typed refusal code java.hierarchy.pullUp.unsupported,
 ...
 * to build/reports/cucumber/java-hierarchy-pull-up-refusal-codes.txt for reconciliation.
 *
 * Slice glue-shared-refusal-r003 (migration to shared refusal glue): the five Scenario 2
 ...
 * KotlinJvmRefusalScenarioContext port). previewPullUpMember publishes the produced plan
 ...
 */
class KotlinJvmHierarchyPullUpRefusalSteps(private val context: KotlinJvmRefusalScenarioContext) {
    ...
    private val refusalCode = "java.hierarchy.pullUp.unsupported"

    // ---------------------------------------------------------- Scenario 1 Given

    @Given("^the Java adapter receives a pullUpMember refactoring request on a Java workspace$")
    fun javaAdapterReceivesPullUpMemberRequest() {
        setupFixture()
        request = RefactoringRequest(
            operation = "pullUpMember",
            snapshot = snapshot,
        )
    }
    ...
    private fun setupFixture() {
        val root = temporaryDirectory("rk-jvm-hierarchy-pull-up-refusal")
    ...
    private fun previewPullUpMember() {
        val snap = snapshot
    ...
    fun cleanup(scenario: Scenario) {
        ...
        val report = reportDir.resolve("java-hierarchy-pull-up-refusal-codes.txt")
