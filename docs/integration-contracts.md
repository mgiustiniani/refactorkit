...
| `refactorkit kotlin symbols <root> ...` / `kotlin definition` / `kotlin references` | `experimental` | Saved-snapshot compiler-proven declaration search, ID definition, direct function/type-usage position definition and explicitly partial references. Type IDs derive from JVM binary identity; function IDs derive from JVM owner/name/descriptor; usage ranges come from K2 FIR real PSI. Exact lease/snapshot authority is mandatory; unsupported or incomplete evidence refuses without fallback. |
