...
| `refactorkit kotlin symbols <root> ...` / `kotlin definition` / `kotlin references` / `kotlin rename` | `experimental` | Saved-snapshot compiler declarations/usages plus bounded K4 private-declaration managed rename. In mixed snapshots, `--accept-external-consumer-risk` admits the first public top-level Kotlin-type preview only when ephemeral K2 class-file identity and exact JDT Java bindings agree and clean staged K2/JDT evidence remains. Lease/snapshot/index authority is mandatory; symmetric Java-to-Kotlin, members and incomplete shapes still refuse without fallback. |
