...
| `intelligence.query` | `experimental` | Request/snapshot-correlated, bounded workspace/document symbol variants are ready. TypeScript/JavaScript document symbols, completion, hover and signature help accept exact lease-bound immutable editor overlays, reject stale versions and restore saved provider documents. Position-navigation variants remain refused; schema: `api-0.2-intelligence-query-schema.json`. |
