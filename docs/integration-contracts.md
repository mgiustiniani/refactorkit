...
| `intelligence.query` | `experimental` | Request/snapshot-correlated, bounded workspace/document symbol variants are ready. TypeScript/JavaScript document symbols, completion, hover and signature help accept exact lease-bound immutable editor overlays, reject stale versions and restore saved provider documents. Saved-snapshot Java definition/references-at-position are backed by a bounded snapshot-keyed JDT binding cache; references expose declaration inclusion and explicit truncation. Other position-navigation variants remain refused. Barrier-aware priorities and `intelligence.cancel` provide cooperative interactive scheduling without crossing stateful operations; schema: `api-0.2-intelligence-query-schema.json`. |
