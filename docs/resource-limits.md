...
| Immutable editor overlay | 128 existing source documents, 786,432 aggregate UTF-8 bytes, 10 MiB/document | Invalid/duplicate/traversing/drive/backslash/mixed-language paths, negative versions and NUL content are refused before provider invocation; provider snapshot is in-memory only |
| Kotlin K2 diagnostics and declaration symbols | 30-second aggregate deadline, at most 2 worker attempts after one transient non-zero infrastructure exit, 512 MiB compiler heap, 8 MiB stdout, 64 KiB stderr, 1 concurrent process, 96 `.kt` files, 128 project classpath entries, 500 diagnostics, 500 compiler symbols, 2,000 resolved function usages, 16 MiB and 10,000 non-synthetic methods/generated callable-owner class, 1,024 characters/JVM descriptor, 4,096 characters/diagnostic message, 512 UTF-16 units/symbol name | Timeout/output/protocol failures do not retry; a repeated worker exit returns a typed error. Every process tree is terminated and no partial diagnostics or symbol index is promoted; protocol symbol output remains capped at 200 with explicit truncation |
