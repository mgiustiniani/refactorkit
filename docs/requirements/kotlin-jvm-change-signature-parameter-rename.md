...
K2 FIR override checking plus resolved source class-supertypes assigns one stable override-family identity. If the target is part of a source override/implementation family, every source declaration at the same ordinal is renamed atomically. Per approved changes 011 and 012, the four non-inducible REQ-001 family-incompleteness criteria — fewer family functions than family parameters, a function and parameter family that disagree, a family crossing an external or unavailable declaration boundary, and a hierarchy member with fewer than two family functions — are superseded and retained only as defensive guards: each returns a successful read-only semantic preview (SEMANTIC_PREVIEW) with a defensive-gate note and no refusal (they are not removed). The inducible "lacking one exact parameter declaration at the selected ordinal" family-incompleteness case is explicitly retained and refuses fail-closed, reporting the typed code kotlin.changeSignatureFamilyIncomplete.
