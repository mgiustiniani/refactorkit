workspace "RefactorKit" "Deterministic refactoring and code-intelligence engine for safe, previewable, rollbackable code transformations." {
    ...
    model {
        ...
        refactorKit = softwareSystem "RefactorKit" "Deterministic refactoring and code-intelligence engine. It produces patch previews, applies safe workspace edits, records transactions, and exposes CLI, JSON-RPC daemon, LSP, MCP, and library integration modes." {
            ...
            daemon = container "JSON-RPC Daemon" "Long-running local process over stdio that keeps a workspace session, exposes project summary, symbol search, diagnostics, refactor preview/apply, and rollback." "Kotlin/JVM application (modules/refactorkit-daemon)"
            ...
            kotlinAdapter = container "Kotlin Language Adapter" "Initial Kotlin module boundary with .kt/.kts routing and explicit refused capabilities until a compiler/Analysis API backend is qualified." "Kotlin/JVM library (modules/refactorkit-kotlin)" {
                tags "Runtime Building Block"
            }
