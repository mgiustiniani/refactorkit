workspace "RefactorKit" "Deterministic refactoring and code-intelligence engine for safe, previewable, rollbackable code transformations." {
    ...
    model {
        ...
        refactorKit = softwareSystem "RefactorKit" "Deterministic refactoring and code-intelligence engine. It produces patch previews, applies safe workspace edits, records transactions, and exposes CLI, JSON-RPC daemon, LSP, MCP, and library integration modes." {
            ...
            jvmComposition = container "Shared JVM Semantic Composition" "Composes exact K2/JDT and bounded annotation-processing-free ECJ/K2 evidence for bidirectional public type/member rename and bounded Kotlin package-move plans without coupling core to either adapter." "Kotlin/JVM library (modules/refactorkit-jvm)" {
                tags "Runtime Building Block"
            }
        ...
        cli -> kotlinAdapter "Configures explicit Kotlin toolchain and requests diagnostics, navigation, and bounded mutation previews" "In-process orchestration + external K2 worker"
        cli -> jvmComposition "Requests mixed Java/Kotlin public rename previews with explicit public-API risk acceptance" "In-process calls"
        ...
        daemon -> jvmComposition "Composes K2/JDT evidence for mixed-language managed previews" "In-process calls"
        ...
        mcp -> kotlinAdapter "Configures and queries compiler-backed Kotlin diagnostics and symbols" "In-process orchestration + external K2 worker"
        mcp -> jvmComposition "Composes K2/JDT evidence for mixed-language managed previews" "In-process calls"
