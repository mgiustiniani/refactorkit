...
Feature: Preserve managed-apply diagnostics-gate selection across daemon and MCP
  ...
  Scenario Outline: Selection case "<case>" follows the first matching ordered route
    Given the selector owns only this precedence-ordered routing table:
      ...
      | 6     | languageId == "kotlin" and not javaAffected after moveDeclaration is excluded                                     | kotlin-k2                  | currentKotlinAdapter.compilerDiagnostics(candidate).diagnostics                           |
    ...
    Examples:
      ...
      | Kotlin-only non-move uses K2 fallback      | kotlin      | renameSymbol                 | NATIVE_AST       | src/main/kotlin/Foo.kt                                              | unused                            | 6   | kotlin-k2                        | lazy compilerDiagnostics result provider of the current Kotlin adapter                   |
