...
Feature: Preserve managed-apply diagnostics-gate selection across daemon and MCP
  ...
  Scenario Outline: Selection case "<case>" follows the first matching ordered route
    Given the selector owns only this precedence-ordered routing table:
      ...
      | 3     | languageId == "kotlin" and operation == "moveDeclaration"                                                        | kotlin-k2-java-jdt         | KotlinJvmMoveDeclarationPlanner(currentKotlinAdapter).diagnostics(candidate)              |
      | 4     | languageId == "kotlin" and operation == "changeSignature.renameParameter"                                         | kotlin-k2-java-jdt-change-signature | KotlinJvmChangeSignaturePlanner(currentKotlinAdapter).diagnostics(candidate)      |
      | 5     | languageId == "kotlin" and javaAffected and operation != "moveDeclaration" and evidence == "JDT_BINDING"         | kotlin-k2-java-jdt         | JavaKotlinPublicTypeRenamePlanner(currentKotlinAdapter).diagnostics(candidate)             |
      | 6     | languageId == "kotlin" and javaAffected after rows 3 through 5 fail                                                 | kotlin-k2-java-jdt         | KotlinJavaPublicTypeRenamePlanner(currentKotlinAdapter).diagnostics(candidate)             |
      | 7     | languageId == "kotlin" and not javaAffected after operation-specific rows are excluded                              | kotlin-k2                  | currentKotlinAdapter.compilerDiagnostics(candidate).diagnostics                           |
      | 8     | every other exact languageId                                                                                         | resolver gate ID unchanged  | externalGateResolver(languageId) return value unchanged                                   |
    ...
    Examples:
      ...
      | Kotlin change signature beats affected-file inference | kotlin | changeSignature.renameParameter | NATIVE_AST   | src/main/kotlin/Foo.kt                                              | unused                            | 4   | kotlin-k2-java-jdt-change-signature | lazy KotlinJvmChangeSignaturePlanner provider using the current Kotlin adapter        |
      | Java-affected non-move JDT binding        | kotlin      | renameClass                  | JDT_BINDING      | src/main/java/com/acme/FooUser.java, src/main/kotlin/Foo.kt         | unused                            | 5   | kotlin-k2-java-jdt               | lazy JavaKotlinPublicTypeRenamePlanner provider using the current Kotlin adapter          |
      | Java-affected broad fallback              | kotlin      | deliberately-unusual         | NATIVE_AST       | src/main/kotlin/Foo.kt, src/main/java/com/acme/FooUser.java         | unused                            | 6   | kotlin-k2-java-jdt               | lazy KotlinJavaPublicTypeRenamePlanner provider using the current Kotlin adapter          |
      | Kotlin-only move keeps operation diagnostics | kotlin   | moveDeclaration              | JDT_BINDING      | src/main/kotlin/Foo.kt                                              | unused                            | 3   | kotlin-k2-java-jdt               | lazy KotlinJvmMoveDeclarationPlanner provider using the current Kotlin adapter           |
      | uppercase JAVA move keeps operation diagnostics | kotlin | moveDeclaration              | JDT_BINDING      | src/main/java/com/acme/Foo.JAVA                                    | unused                            | 3   | kotlin-k2-java-jdt               | lazy KotlinJvmMoveDeclarationPlanner provider using the current Kotlin adapter           |
      | Kotlin-only non-move uses K2 fallback      | kotlin      | renameSymbol                 | NATIVE_AST       | src/main/kotlin/Foo.kt                                              | unused                            | 7   | kotlin-k2                        | lazy compilerDiagnostics result provider of the current Kotlin adapter                   |
      | case-changed Java ID is external          | Java        | java.moveAcrossMavenModules  | JDT_BINDING      | src/main/java/com/acme/Foo.java                                     | external-case-sensitive-java-v1  | 8   | external-case-sensitive-java-v1 | the exact resolver-returned gate object unchanged                                        |
      | case-changed Kotlin ID is external        | Kotlin      | moveDeclaration              | JDT_BINDING      | src/main/java/com/acme/FooUser.java                                 | external-case-sensitive-kotlin-v1 | 8   | external-case-sensitive-kotlin-v1 | the exact resolver-returned gate object unchanged                                        |
      | ordinary external ID is opaque            | typescript  | rename                       | LANGUAGE_SERVER  | src/Foo.ts                                                          | typescript-compiler-exact-v1      | 8   | typescript-compiler-exact-v1     | the exact resolver-returned gate object unchanged                                        |
